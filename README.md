...
После прохождения постгресом хэлсчека, сервис запустится на порту `8080` и будет готов принимать запросы. Проверить работоспособность можно, например, открыв `http://localhost:8080/swagger/`, где должен открыться доступ к Swagger

# Примеры запросов

Примеры некоторых запросов находятся в

- **Запрос создания сегмента**

    `POST http://localhost:8080/api/segment`

    ![Картинка с примером запроса создания сегмента](image.png)
    Принимает название сегмента

    ![Картинка с некорректным запросом](image-1.png)
    Если передан не slug отправляет статус код Unprocessable Entity

    ![Картинка с некорректным форматом запроса](image-4.png)
    ...
    ![Картинка с попыткой добавить уже существующий сегмент](image-3.png)
...
- **Запрос создания сегмента с автоматическим добавлением заданного процента пользователей в него**
    ...
    ![Картинка с примером запроса создания сегмента](image-35.png)
    ...
    ![Картинка с попыткой добавить сегмент, с некорректным процентом пользователей](image-6.png)
    На картинке процент 150, поэтому вернулся статус код Bad Request

    ![Картинка с попыткой добавить сегмент, с некорректным процентом пользователей](image-7.png)
...
- **Запрос удаления сегмента из списка доступных**
    ...
    ![Картинка с примером запроса удаления сегмента](image-8.png)
    ...
    ![Картинка с запросом удаления несуществующего сегмента](image-9.png)
    Если переданный сегмент не существует - статус код Not Found

    ![Картинка с некорректным форматом запроса](image-12.png)
...
- **Запрос обновления (добавления, удаления) сегментов пользователя**
    ...
    ![Запрос добавления пользователя](image-13.png)
...
- **TTL сегментов пользователя**
    ...
    ![Запрос с TTL сегментов](image-18.png)
    ...
    ![Некорректный формат времени](image-19.png)
...
- **Запрос чтения активных сегментов пользователя**
    ...
    ![Запрос чтения активных сегментов пользователя](image-20.png)
    ...
    ![Запрос чтения сегментов несуществующего пользователя](image-21.png)
    Если пользователя с запрашиваемым id нет - выдает Not Found

    ![Запрос чтения сегментов пользователя, у которого их нет](image-22.png)
...
- **Запрос формирования отчета по добавлениям/удалениям пользователя из сегментов**
    ...
    ![Простой запрос формирования отчета](image-23.png)
    ...
    ![Запрос формирования отчета по конкретному месяцу](image-25.png)
    Указав query параметр `exact` можно получить ссылку на отчет по конкретному месяцу. Формат времени указывается как `ГГГГ-ММ`

    ![Запрос формирования отчета по конкретный месяц](image-26.png)
    ...
    ![Запрос формирования отчета с конкретного месяца](image-27.png)
...
- **Запрос чтения отчета по истории добавлений/удалений пользователя из сегментов**
    ...
    ![Несуществующий отчет](image-34.png)
